...
1. 下载并验证语言模型：`powershell -ExecutionPolicy Bypass -File pipelines/foundation/download_language_model.ps1`
...
3. 样本验证：`powershell -ExecutionPolicy Bypass -File pipelines/foundation/run_text_profile.ps1 -Mode sample`
4. 全量运行：`powershell -ExecutionPolicy Bypass -File pipelines/foundation/run_text_profile.ps1 -Mode full`

模型为 fastText 官方 `lid.176.bin`，131,266,198 字节，许可 CC-BY-SA-3.0；来源、大小与
SHA-256 保存于 `pipelines/foundation/language-model-lock.json`。下载器验证通过后复用已有文件。
