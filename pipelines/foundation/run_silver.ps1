...
$submitArgs = @('--master','spark://spark-master:7077','--deploy-mode','client','--driver-memory','2g',
    ...
    '--py-files','/pipelines/foundation/cleaning.py,/pipelines/foundation/silver_job.py')
if ($Mode -eq 'fixture') {
    $submitArgs += '/pipelines/foundation/fixture_spark.py'
    ...
    $submitArgs += @('/pipelines/foundation/replay_check.py','--left',"/data/silver/$LeftRunId",'--right',"/data/silver/$RightRunId",'--output',"/data/silver/$RunId.json")
    if ($Subset) { $submitArgs += '--subset' }
} else {
    $submitArgs += @('/pipelines/foundation/silver_job.py','--mode',$Mode,'--run-id',$RunId)
}
...
foreach ($sourcePath in @('infra/compose.yaml','infra/compose.silver.yaml','pipelines/foundation/run_silver.ps1','infra/spark/connector-jars.sha256')) {
