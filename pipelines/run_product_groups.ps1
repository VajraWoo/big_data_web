...
& $dockerPath @composeArgs run --rm --name $driverName --no-deps spark-driver --master spark://spark-master:7077 --deploy-mode client --driver-memory 2g --executor-memory 3g --conf "spark.driver.host=$driverName" --conf spark.driver.bindAddress=0.0.0.0 --conf spark.executor.cores=2 --conf spark.cores.max=4 --py-files /pipelines/product_groups.py /pipelines/product_group_job.py --run-id $RunId 2>&1 | Tee-Object -FilePath "docs/runs/evidence/$RunId.log"
